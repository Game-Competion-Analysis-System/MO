...
export default function AnalysizesScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!found) return;
        const list = await apiGet<ServerDto[]>(
          `/api/servers/game/${found.gameId}`,
        );
        setServers(list || []);
      } catch {}
    })();
  ...
  const filteredServers = useMemo(
    () =>
      serverSearch.trim()
        ? servers.filter((s) =>
            s.serverName?.toLowerCase().includes(serverSearch.toLowerCase()),
          )
        : servers,
  ...
  async function checkRankChanges(newAnalysis: AnalysisSummary) {
    ...
    const previous = (page.items || [])
      .filter(
        (a) => a.gameName === game && a.analysisId !== newAnalysis.analysisId,
      )
      .at(0);
  ...
  async function pickImage() {
    ...
    try {
      ...
      const analysis = await apiPostForm<AnalysisSummary>(
        "/api/ai/analyze",
        formData,
        true,
      );
    ...
    <ScrollView contentContainerStyle={[container.padding, container.gap]}>
            ...
            <View style={styles.serverDropdown}>
              {filteredServers.slice(0, 8).map((s) => (
                <Pressable
                  ...
                  style={[
                    styles.serverItem,
                    selectedServer === s.serverName && styles.serverItemActive,
                  ]}
                  onPress={() => {
                ...
                >
                  <Text
                    style={[
                      ...
                      selectedServer === s.serverName &&
                        styles.serverItemTextActive,
                    ]}
                  >
                    {s.serverName}
                  </Text>
      ...
      {analysisState === "idle" ? (
        <Pressable
          style={[
            styles.fileSelector,
            !selectedServer && styles.fileSelectorDisabled,
          ]}
          onPress={() => {
            if (!selectedServer) {
        ...
        >
          ...
          <Text
            style={[
              { fontWeight: "bold" },
              !selectedServer && styles.disabledText,
            ]}
          >
      ...
      {analysisState === "complete" && result && (
        <>
          ...
          {(result.serverName || selectedServer) && (
            <Text style={headers.h4}>
              Server: {result.serverName || selectedServer}
            </Text>
          )}
          {result.eventName && (
            <Text style={headers.h4}>Event: {result.eventName}</Text>
          )}
          ...
          {leaderboardEntries === null ? (
            ...
            <View style={styles.emptyBox}>
              <Text style={headers.h4}>
                No leaderboard entries found for this analysis.
              </Text>
            </View>
          ) : (
